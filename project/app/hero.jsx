// Hero with email expand → survey transition + tweaks
...
function Hero({ tweaks, onStart }) {
  const [emailValue, setEmailValue] = useStateH("");
  const [domainValue, setDomainValue] = useStateH("");
      ...
      <div className="shell">
        ...
        <div className="hero-grid" style={{
          ...
          <div className="enter">
            ...
            {/* Inline form: domain + email side-by-side, clean editorial */}
            <div className="card" style={{padding: 24, background:'var(--card)'}}>
              <div className="mono" style={{fontSize:10.5, letterSpacing:'0.18em', textTransform:'uppercase', color:'var(--ink-3)', marginBottom:16, display:'flex', justifyContent:'space-between'}}>
                <span><span className="dot" style={{display:'inline-block', width:6, height:6, background:accent, borderRadius:'50%', verticalAlign:1, marginRight:8}}/>Step 1 of 3 · Website</span>
                <span>~ 2 min</span>
              </div>
            ...
            {tweaks.showPricingHint && (
              <div className="mono" style={{marginTop:24, fontSize:11.5, letterSpacing:'0.14em', textTransform:'uppercase', color:'var(--ink-3)'}}>
                Activation after report · $100 first month · then $170/mo · cancel anytime
              </div>
            )}
          </div>
