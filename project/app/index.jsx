// Top-level app: orchestrates landing + funnel modal screens + tweaks panel
const { useState: useStateA, useEffect: useEffectA } = React;

function App() {
  const [tweaks, setTweak] = useTweaks(TWEAK_DEFAULTS);
  const [screen, setScreen] = useStateA('landing'); // landing | survey | scanning | report | activation | done
  const [data, setData] = useStateA({ domain:'', email:'' });
  const [curtain, setCurtain] = useStateA(true);
  React.useEffect(() => {
    const t = setTimeout(() => setCurtain(false), 1300);
    return () => clearTimeout(t);
  }, []);

  // Keep accent in sync via CSS var
  useEffectA(() => {
  ...
  return (
    ...
    <div data-screen-label={
      ...
      screen === 'survey' ? '02 Survey' :
      screen === 'scanning' ? '03 Scanning' :
      screen === 'report' ? '04 Report' :
      screen === 'activation' ? '05 Activation' : '06 Confirmation'
    }>
      ...
      {(screen === 'survey' || screen === 'scanning' || screen === 'report' || screen === 'activation' || screen === 'done') && (
        <FunnelStage>
          {screen === 'survey' && (
            <Survey
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection title="Funnel">
          ...
          <TweakButton onClick={() => { setData({domain:'northwindrobotics.com', email:'sam@northwindrobotics.com', company:'Northwind Robotics', service:'Warehouse automation', market:'Mid-market 3PLs', goal:'Show up better in AI search'}); setScreen('report'); }}>
            Jump to report
          </TweakButton>
          <TweakButton onClick={() => { setData({domain:'northwindrobotics.com', email:'sam@northwindrobotics.com'}); setScreen('activation'); }}>
            Jump to activation
          </TweakButton>
          <TweakButton onClick={() => { setData({domain:'northwindrobotics.com', email:'sam@northwindrobotics.com'}); setScreen('done'); }}>
